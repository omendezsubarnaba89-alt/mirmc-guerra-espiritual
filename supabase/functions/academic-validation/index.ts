...
Deno.serve(async(req:Request)=>{
  ...
  let body:any={};try{body=await req.json();}catch{return reply({error:'invalid_json'},400,origin)}const action=String(body?.action||'');

  const lessonPassed=async(key:string)=>{const {data}=await admin.from('lesson_validations').select('passed').eq('user_id',user.id).eq('lesson_key',key).maybeSingle();return Boolean(data?.passed);};
  const examPassed=async(level:number)=>{const {data}=await admin.from('exam_validations').select('passed').eq('user_id',user.id).eq('level',level).maybeSingle();return Boolean(data?.passed);};
  const effectiveLessonKey=async(key:string)=>{
    const fallback=LESSON_KEYS[key];if(!fallback)return null;
    const {data,error}=await admin.from('content_items').select('published_payload,archived').eq('content_type','lesson').eq('content_key',key).maybeSingle();
    if(error||!data||data.archived||!data.published_payload)return fallback;
    const quiz=data.published_payload?.quiz;
    if(!Array.isArray(quiz)||quiz.length!==3)return fallback;
  ...
  if(action==='status'){
    ...
    if(lessonError||examError)return reply({error:'status_failed'},500,origin);return reply({lessons:lessons||[],exams:exams||[]},200,origin);
  }

  if(action==='submit_lesson'){
    const key=String(body?.lesson_key||'');const answerKey=await effectiveLessonKey(key);const answers=Array.isArray(body?.answers)?body.answers.map(Number):[];
    if(!answerKey||answers.length!==answerKey.length||answers.some((x:number)=>!Number.isInteger(x)||x<0||x>20))return reply({error:'invalid_lesson_submission'},400,origin);
    const n=Number(key);
    if(n>1){
    ...
    const score=answers.reduce((sum:number,value:number,index:number)=>sum+(value===answerKey[index]?1:0),0);const passed=score>=2;const now=new Date().toISOString();
    ...
    const {error}=await admin.from('lesson_validations').upsert(next,{onConflict:'user_id,lesson_key'});if(error)return reply({error:'lesson_record_failed'},500,origin);
